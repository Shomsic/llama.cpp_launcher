...
class BenchmarkTab:
    def __init__(self, parent, state, toast, app):
        self.parent_frame = parent
        self.state = state
        self.toast = toast
        
        # Robust app reference: ensure we have the LlamaLauncherApp instance
        actual_app = app
        if not hasattr(actual_app, 'run_ai_tune'):
            current = app
            found = False
            while current:
                if hasattr(current, 'run_ai_tune'):
                    actual_app = current
                    found = True
                    break
        ...
        self.ai_tune_rounds_var = tk.IntVar(value=8)
        self.ai_tune_retune_var = tk.BooleanVar(value=False)
    ...
    def _create_ui(self):
        ...
        model_section = ttk.LabelFrame(left_panel, text=" Модель для теста ", padding=10)
        model_section.pack(fill=X, pady=(0, 10))
        
        self.bench_model_var = tk.StringVar(value=self.state.active_model_var.get())
        model_combo = ttk.Combobox(model_section, textvariable=self.bench_model_var, 
                                   values=self.state.get_all_models(), state="readonly")
        model_combo.pack(fill=X, expand=True)
        model_combo.bind("<<ComboboxSelected>>", self._on_bench_model_changed)
        ...
        # AI‑Tune rounds
        ttk.Label(params_section, text="AI‑Tune rounds:").grid(row=4, column=0, sticky=W, pady=5)
        self.ai_rounds_spin = ttk.Spinbox(params_section, from_=1, to=20, textvariable=self.ai_tune_rounds_var, width=10)
        self.ai_rounds_spin.grid(row=4, column=1, sticky=E, pady=5)
        
        # Retune (ignore cache)
        self.retune_check = ttk.Checkbutton(params_section, text="Retune (ignore cache)", variable=self.ai_tune_retune_var)
        self.retune_check.grid(row=5, column=0, columnspan=2, sticky=W, pady=5)
        ...
        self.tune_btn = ttk.Button(left_panel, text="🪄 AI-Tune", style="Header.TButton", command=lambda: self.app.run_ai_tune(self))
        self.tune_btn.pack(fill=X, pady=(5, 0))
    ...
    def _on_bench_model_changed(self, _event=None):
        model_path = self.bench_model_var.get()
        self.model_info_label.config(text=f"Модель: {os.path.basename(model_path)}")
    ...
    def _on_bench_output(self, line):
        self.app.root.after(0, lambda: self._append_log(line))
    ...
    def _on_bench_finished(self):
        self.app.root.after(0, self._ui_on_finished)
